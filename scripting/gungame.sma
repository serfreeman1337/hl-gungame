...
public CMD_GGToggle(id,level,cid){
	...
	new adminName[32],adminAuth[36],adminIp[15]
...
public load_cfg(){
	new mapName[32],cfgFilePath[512],cfgFileLen,bool:isMapCfg
	...
	if(f){
		...
		while(!feof(f)){
			...
			switch(currentBlockId){
				...
				case CFG_BLOCK_CVARS:{ // parse cvar block
					if(buffer[0] == '<'){
						if(strcmp(buffer,"</cvars>") == 0)
				...
				case CFG_BLOCK_WEAPONS:{ // parse weapons block
				...
				case CFG_BLOCK_MODSETTINGS:{ // parse mod settings
					if(buffer[0] == '<'){
					...
					switch(modBlockId){
						...
						case CFG_MOD_OFFSETS:{ // parse offsets block
						...
						case CFG_MOD_BLOCKSPAWN:{
							if(strcmp(buffer,"</blockspawn>") == 0){ // block spawn parse finished
								modBlockId = CFG_MOD_NONE
								continue
							}
...
Map_LockItems(bool:lock){
	...
	for(new ent = MaxClients +  1 ; ent <= entCount ; ent++){
		if(is_valid_ent(ent) && Item_LockCheck(ent)){ // find our blocked entity
...
Item_LockCheck(ent){
	new classname[32],checkClassName[32]
	entity_get_string(ent,EV_SZ_classname,classname,charsmax(classname))
...
public client_putinserver(id){
	...
	if(!playersData[id][PLAYER_BOT]){
		...
		if(autoSaveTime && autoSaveMap){
			new authId[36],aSave[autoSaveStruct]
		...
		if(warmUpMode == 0 && get_pcvar_num(cvar[CVAR_WARMUP])){ // start warmup mode
			warmUpMode = 1
...
public SetLevel_ForNewPlayer(id,handicap){
	...
	switch(handicap){
		case 1:{ // calculate average level
			new lvlSum
...
public On_PlayerKilled(victim,killer){
	...
	if(playersData[killer][PLAYER_KILLS] >= playersData[killer][PLAYER_NEEDKILLS]){ // level up
		...
		if(get_pcvar_num(cvar[CVAR_TEAMPLAY])){
			if(playersData[killer][PLAYER_CURRENTLEVEL] != maxLevel)
				gg_notify_msg(killer,
					playersData[killer][PLAYER_CURRENTLEVEL] != maxLevel - 1 ? NOTIFY_UP: NOTIFY_LAST
				)
...
public Equip_PlayerWithWeapon(const id){
	...
	if(isTeamPlay && warmUpMode != 1){
		...
		if(playersData[id][PLAYER_TEAM] != teamIndex){	// recheck data for matching team stats
			new players[32],pnum
			get_players(players,pnum,"e",teamName)
			
			if(pnum > 1){	// can get valid inf
				for(new i,player ; i < pnum ; i++){
					player = players[i]
	...
	if(playersData[id][PLAYER_CURRENTLEVEL] != playersData[id][PLAYER_LASTLEVEL] && warmUpMode == 2){
		...
		if(!isVoteStarted &&
			get_pcvar_num(cvar[CVAR_MAPCHANGES_STYLE]) == 2 && 
			(playersData[id][PLAYER_CURRENTLEVEL] > maxLevel - get_pcvar_num(cvar[CVAR_NEARLEVEL]))
		){
			start_map_vote(0.0)
		}
		...
		if(playersData[id][PLAYER_CURRENTLEVEL] > playersData[id][PLAYER_LASTLEVEL] && playersData[id][PLAYER_LASTLEVEL] != -1){ // do levelup stuff
			if(sound_levelup[0])
			...
			if(!isTeamPlay){
				if(!levelSkipped){
					gg_notify_msg(id,
						playersData[id][PLAYER_CURRENTLEVEL] != maxLevel - 1 ? NOTIFY_UP : NOTIFY_LAST
					)
				}else{
		...
		}else if(playersData[id][PLAYER_CURRENTLEVEL] < playersData[id][PLAYER_LASTLEVEL]){ // do leveldown stuff
		...
		copy(playersData[id][PLAYER_ICON],
			charsmax(playersData[]),
	...
	if(!playersData[id][PLAYER_NOREFIL] && weaponSet[WSET_INFARRAY]){
		new inflictor[32],inflictorAmmo[weaponAmmoStruct]
	...
	}else if(weaponSet[WSET_INFARRAY]){ // read ammo cache for level inflictors
		new inflictor[32],inflictorData[4]
		...
		for(new i,infSize = ArraySize(weaponSet[WSET_INFARRAY]) ; i < infSize ; i++){
			...
			set_pdata_int(id,
				modOffsets[m_rgAmmo] + inflictorData[2] - modOffsets[offsetAmmoDiff],
				inflictorData[0]
			)
		}
	}
	
	if(!playersData[id][PLAYER_NOREFIL] && weaponSet[WSET_NOREFIL_MAP]){ // update no ininity ammo map
...
public Add_PlayerWeapon(id,equipItem[equipStruct],weaponSet[weaponSetStruct]){
	...
	for(new i ; i < 2 ; ++i){ // ammo functions
		if(weaponAmmo[WAMMO_PRIMARY_AMMOID + i] == -1)
			continue
		...
		if(equipItem[EQUIP_PRIMARY_REFIL_TIME + i] > 0.0){ // set ammo refil tasks
...
public MSG_AmmoX(MSG_Dest,MSG_ID,id){
	...
	if(ammoAmmount < maxAmmo){ // reset to max ammo
...
public gg_notify_msg(id,notifyId){
	...
	new players[32],pnum,name[32],message[128],len
	...
	for(new i,player ; i < pnum ; i++){
		...
		switch(notifyId){
			case NOTIFY_TEAMUP,NOTIFY_TEAMWIN,NOTIFY_TEAMLAST: { // team messages
				#if defined AGHL_COLOR
...
public Inflictors_SpawnHandler(ent){
	...
	new entClassname[32],inflictorData[2]
	entity_get_string(ent,EV_SZ_classname,entClassname,charsmax(entClassname))
	
	if(!TrieGetArray(playersData[owner][PLAYER_INFLICTORS],entClassname,inflictorData,sizeof inflictorData)){ // check if this inflictor is exists in map and get overall limit
		set_task(0.1,"ENT_DelayedRemove",task_EntRemove_Id + ent)
...
public Inflictors_DamageHandler(ent,inflictor,attacker){
	...
	new classname[32],inflictorData[2],owner = entity_get_int(ent,EV_INT_iuser3)
	entity_get_string(ent,EV_SZ_classname,classname,charsmax(classname))
...
public goto_nextmap(){
	if(sound_winner[0]){
		if(containi(sound_winner,".mp3") != -1) client_cmd(0,"mp3 stop",sound_winner)
		else client_cmd(0,"stopsound",sound_winner)
	}
	
	new nextMap[32]
	get_cvar_string("amx_nextmap",nextMap,31)
	
	if(!nextMap[0]){ // force change by hlds
...
// Конец игры. Отображение победителя, воспроизведение музыки и тп	// Да ладно?
public endgame(id){
	new winnerName[32]
	...
	if(sound_winner[0]){		
		if(containi(sound_winner,".mp3") != -1) client_cmd(0,"mp3 play ^"%s^"",sound_winner)
		else client_cmd(0,"spk ^"%s^"",sound_winner)
	}
	...
	if(get_pcvar_num(cvar[CVAR_SHOWSTATS])){ // shows endgame stats
	...
	UTIL_ScreenFade(0, {0,0,0}, endgame_delay, 120.0, 255, 1)
...
public api_get_level_data(plugin,params){
	...
	set_array(2,weaponSet,weaponSetStruct - 1)
...
public api_get_player_level(plugin,params){
	...
	set_array(2,playersData[player],playersDataStruct - 1)
